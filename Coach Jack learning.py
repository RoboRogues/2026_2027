...
class VexRobot:
    ...
    def any_bump_pressed(self) -> bool:
        return any([
            ...
            self.bump_rl.is_pressed(),
            self.bump_rr.is_pressed()
        ])
    ...
    def select_alliance_tag(self, team_color: str, seen_tags: List[str]) -> Optional[str]:
        ...
        prefs = alliance_map.get(team_color.lower(), alliance_map["blue"])
        for tag in seen_tags:
            for p in prefs:
                if str(tag).upper().startswith(p.upper()):
                    return tag
        return None

    def run_auton_15s(self):
        start = time.time()
        time_limit = 15.0

        seen_tag = self.vision.get_apriltag()
        team_color = getattr(self.radio, "team_color", None) or self.select_pin_color(timeout=1.0)
        alliance_tag = None
        if seen_tag:
            alliance_tag = self.select_alliance_tag(team_color, [seen_tag])

        if alliance_tag:
            tag_x, tag_y = self.gps.get_position()
            waypoints = [(tag_x, tag_y), (tag_x + 0.3, tag_y)]
        else:
            tag = self.gps.read_field_tag()
            if tag == "zone_a":
                waypoints = [(0.5, 0.0), (0.5, 0.5)]
            elif tag == "zone_b":
                waypoints = [(0.3, -0.2), (0.6, 0.2)]
            else:
                waypoints = [(0.6, 0.0), (0.6, 0.4)]

        pin_color = self.select_pin_color(timeout=1.5)
        if pin_color == "red":
            waypoints = [(x, y + 0.05) for (x, y) in waypoints]
        elif pin_color == "blue":
            waypoints = [(x, y - 0.05) for (x, y) in waypoints]

        # optional: short autotune (bench only)
        try:
            best_params, best_score = self.drive_autotune_fuzzy(waypoints[0][0], waypoints[0][1], trials=4, trial_time=1.0)
            # logging placeholder: print(best_params, best_score)
        except Exception:
            pass
        ...
        for (tx, ty) in waypoints:
            ...
            if remaining <= 0:
                break
            per_leg_timeout = min(remaining, 8.0)
            self.drive.drive_to(tx, ty, timeout=per_leg_timeout)
            if self.any_bump_pressed():
                break
    ...
    def drive_autotune_fuzzy(self, target_x: float, target_y: float,
        ...
        for _ in range(trials):
            cand_fb = max(0.0, base_fb + random.uniform(-0.8, 0.8))
            cand_ha = max(0.0, base_ha + random.uniform(-0.8, 0.8))
            self.drive.fuzzy.max_forward_boost = cand_fb
            self.drive.fuzzy.max_heading_adjust = cand_ha
            ...
            while time.time() - start < trial_time:
                ...
                forward_cmd = self.drive.distance_pid.update(distance, dt)
                turn_cmd = self.drive.heading_pid.update(err_heading, dt)
                left_v = forward_cmd - (turn_cmd * self.drive.wheel_base_m / 2.0)
                right_v = forward_cmd + (turn_cmd * self.drive.wheel_base_m / 2.0)
                left_v, right_v = self.drive.fuzzy.adjust(left_v, right_v, distance, err_heading)
                self.drive.tank_drive_voltage(left_v, right_v)

                score += distance * dt + (abs(err_heading) / 180.0) * dt
                time.sleep(0.02)

            self.drive.stop()
            if score < best_score:
                best_score = score
                best_params = (cand_fb, cand_ha)

        self.drive.fuzzy.max_forward_boost, self.drive.fuzzy.max_heading_adjust = best_params
...
def print_scrolling_in_brain(text: str, cols: int = 36, rows: int = 8, delay: float = 0.08, loops: int = 1):
    ...
    lower = "  \\  '-._____.-'  /  "
    bot = "   '-._________.-'   "
    nsteps = max(1, (len(stream) - rows)) * loops

    for step in range(nsteps):
        window = stream[step: step + rows]
        frame = []
        frame.append(top.center(cols + 8))
        frame.append(upper.center(cols + 8))
        frame.append(mid_top.center(cols + 8))
        for ln in window:
            content = ln[:cols].ljust(cols)
            frame.append(("|  " + content + "  |").center(cols + 8))
        frame.append(mid_bottom.center(cols + 8))
        frame.append(lower.center(cols + 8))
        frame.append(bot.center(cols + 8))
        os.system("cls")
        print("\n".join(frame))
        time.sleep(delay)
